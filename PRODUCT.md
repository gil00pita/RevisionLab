...
No global installation is required. Run the installer from an existing Next.js 15/16 App Router project using React 19 and Node.js 20.9 or newer. `--help` displays usage, `init --dry-run` previews changes without writing or installing, and `init --protect` also generates the optional prototype access gate (Next.js 15.5+). The published executable must start through `npx`, dry-run must leave the host unchanged, and initialization must install the exact invoked package version and generate the existing integration described below.
