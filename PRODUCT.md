...
The published installer is available through `npx revisionlab@latest init`; no global installation is required. Run it from an existing Next.js 15/16 App Router project using React 19 and Node.js 20.9 or newer. `--help` displays usage, `init --dry-run` previews changes without writing or installing, and `init --protect` also generates the optional prototype access gate (Next.js 15.5+). Acceptance criteria: the published executable starts through `npx`, dry-run leaves the host unchanged, and initialization installs the exact invoked package version and generates the existing integration described below.
